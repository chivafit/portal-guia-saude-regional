...
export default async function CompaniesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <><main>
    ...
    <section className="section shell" style={{ paddingBottom: 0 }}><div className="company-category-grid">{categories.slice(0, 6).map((item) => <Link key={item.key} href={`/empresas?categoria=${encodeURIComponent(item.key)}`}><Building2 size={20} /><strong>{item.label}</strong><small>{item.count} {item.count === 1 ? "serviço" : "serviços"}</small></Link>)}</div></section>

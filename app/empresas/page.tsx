...
export default async function CompaniesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <><SiteHeader /><main>
    ...
    <nav className="company-reference-categories shell" aria-label="Categorias de serviços">{categories.slice(0,6).map(item=><Link key={item.key} href={`/empresas?categoria=${encodeURIComponent(item.key)}`}><Building2/><strong>{item.label}</strong><small>{item.count} locais</small></Link>)}</nav>

...
export default async function CityPage({ params, rootLanding = false }: { params: Promise<{ slug: string }>; rootLanding?: boolean }) {
  ...
  const featuredProfessionals = localProfessionals
    .filter((item) => isPodcastProfessional(item.slug))
    .map((item) => ({
      slug: item.slug,
      name: item.name,
      specialty: item.specialty,
      organization: item.organization,
      registration: confirmedRegistrationLabel(item.registration),
  ...
  return (
    <>
      ...
      <main className={rootLanding ? "root-guide-home" : undefined}>
        {rootLanding ? (
          <section className="root-guide-hero"><div className="shell root-guide-hero-inner"><p className="eyebrow">Guia Saúde · Portal Regional</p><h1>Encontre profissionais de saúde perto de você</h1><p>Pesquise por nome, profissão, especialidade, clínica ou serviço e encontre informações de saúde na sua região.</p><RootGuideSearch professions={professions} categories={serviceCategories} /><div className="root-guide-quick-searches"><span>Buscas rápidas:</span>{quickSearches.map((search) => <Link key={search.label} href={search.href}>{search.label}</Link>)}</div><p className="root-guide-trust">Informações, endereço e formas de contato reunidos para facilitar sua busca.</p></div></section>
        ) : <section className="city-human-hero city-piumhi-banner"><div className="shell city-human-hero-grid"><div className="city-landing-copy"><p className="eyebrow">Piumhi, Minas Gerais</p><h1>Encontre saúde e cuidado perto de você</h1><p>Profissionais, clínicas, serviços e conteúdos de saúde reunidos em um só lugar.</p><form className="city-search-panel city-search-panel-hero" action="/buscar"><input type="hidden" name="cidade" value={city.name} /><label><span><Search size={16} /> Profissional, especialidade ou serviço</span><input name="q" placeholder="Profissional, especialidade ou serviço" /></label><button type="submit">Buscar <ArrowRight size={16}/></button></form></div></div></section>}
        ...
        <section className="shell content-section city-portal-section">
          <div className="city-pro-directory" id="profissionais"><div className="city-pro-main"><div className="city-block-head"><p className="eyebrow">Profissionais em destaque</p><h2>Profissionais em destaque</h2><span className="city-featured-criterion">Conheça profissionais que participaram do podcast Guia Saúde</span></div>
            {featuredProfessionals.length ? <FeaturedProfessionalsRotator professionals={featuredProfessionals} /> : <div className="city-empty-card"><strong>Base da cidade em formação</strong><p>Os profissionais desta cidade serão organizados por especialidade, com fonte e contato revisados.</p><Link href="/inclusao">Indicar um profissional →</Link></div>}
            <Link className="city-list-all" href={`/buscar?cidade=${cityQuery}&tipo=professionals`}>Ver todos os profissionais <ArrowRight size={14} /></Link></div>
            <aside className="city-sell-side" id="servicos"><p className="city-side-label">Espaços comerciais</p>{commercialPharmacy ? <div className="city-sponsor-card"><span><Pill size={13} /> Farmácia em destaque</span><strong>{commercialPharmacy.name}</strong><small>{commercialPharmacy.address}</small><Link href={commercialPharmacy.mapUrl}>Ver localização</Link></div> : <Link href="/anuncie" className="city-sponsor-card empty"><span><Pill size={13} /> Farmácia</span><strong>Espaço para farmácia</strong><small>Sua farmácia em destaque nesta página.</small></Link>}{commercialOptical ? <div className="city-sponsor-card"><span><Glasses size={13} /> Ótica em destaque</span><strong>{commercialOptical.name}</strong><small>{commercialOptical.address}</small><Link href={commercialOptical.mapUrl}>Ver localização</Link></div> : featuredClinic ? <div className="city-sponsor-card"><span><Building2 size={13} /> Clínica em destaque</span><strong>{featuredClinic.name}</strong><small>{featuredClinic.services.slice(0, 2).join(" · ")}</small><Link href={`/buscar?cidade=${cityQuery}&tipo=services`}>Ver detalhes</Link></div> : null}<Link href="/anuncie" className="city-sponsor-anuncie"><Megaphone size={18} /><strong>Quer aparecer aqui?</strong><small>Espaço para destaques, banners e conteúdo patrocinado.</small><em>Falar com o comercial <ArrowRight size={13} /></em></Link></aside></div>

          <section className="city-local-partners" aria-labelledby="city-partners-title"><div><p className="eyebrow">Parceiros e serviços locais</p><h2 id="city-partners-title">Serviços que também fazem parte da cidade</h2></div><div className="city-partner-grid">{commercialPharmacy ? <article><span>Serviços locais em destaque</span><Pill size={19}/><strong>{commercialPharmacy.name}</strong><p>{commercialPharmacy.address}</p><Link href={commercialPharmacy.mapUrl}>Ver localização <ArrowRight size={13}/></Link></article> : null}{commercialOptical ? <article><span>Serviços locais em destaque</span><Glasses size={19}/><strong>{commercialOptical.name}</strong><p>{commercialOptical.address}</p><Link href={commercialOptical.mapUrl}>Ver localização <ArrowRight size={13}/></Link></article> : null}<Link href="/anuncie" className="city-partner-cta"><Megaphone size={19}/><strong>Quer aparecer no Guia Saúde?</strong><p>Mostre sua marca para quem procura atendimento em {city.name}.</p><em>Conhecer opções <ArrowRight size={13}/></em></Link></div></section>

          {editorialArticle ? <section className="city-content-direct" id="materias"><div><p className="eyebrow">Informação</p><h2>Conteúdo para cuidar melhor de você</h2><p>Orientações e notícias para quem vive em {city.name}.</p></div><div className="city-editorial-mosaic"><article className="city-content-editorial-card city-editorial-article" style={{ backgroundImage: `linear-gradient(90deg,rgba(9,43,40,.86),rgba(9,43,40,.22)),url(${articleImage(editorialArticle) ?? "/materias/radiologia-odontologica-diagnostico.jpg"})` }}><span>{editorialArticle.category}</span><strong>{editorialArticle.title}</strong><p>{editorialArticle.excerpt}</p><Link href={`/materias/${editorialArticle.slug}`}>Ler matéria <ArrowRight size={14} /></Link></article><Link href="/podcast" className="city-editorial-podcast" style={{ backgroundImage: `linear-gradient(0deg,rgba(11,61,57,.78),rgba(11,61,57,.08)),url(${latestEpisode.imageUrl})` }}><span><Podcast size={13}/> Podcast</span><strong>Conexão Saúde</strong><p>{latestEpisode.topic}</p><em>Ouvir episódio <ArrowRight size={13}/></em></Link><Link href={`/revista/${readableEdition.slug}`} className="city-editorial-magazine"><span><BookOpen size={13}/> Revista</span>{readableEdition.coverUrl ? <img src={readableEdition.coverUrl} alt={`Capa da ${readableEdition.number} edição da Guia Saúde`} /> : null}<div><strong>{readableEdition.number} edição</strong><p>{readableEdition.title}</p><em>Folhear revista <ArrowRight size={13}/></em></div></Link></div></section> : null}

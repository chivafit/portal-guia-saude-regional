...
export default async function SearchPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><SiteHeader /><main><section className="directory-hero"><div className="shell"><div className="directory-hero-grid"><div><p className="eyebrow">Diretório regional</p><h1>Encontre profissionais da saúde</h1><p>Busque por especialidade, cidade ou serviço. O Guia Saúde mostra perfis e informações revisadas, sem intermediação de agenda.</p></div><div className="directory-note"><span>Portal informativo</span><strong>Sem agendamento online</strong><p>O contato e as condições de atendimento aparecem no perfil de cada profissional.</p></div></div><SearchForm compact /></div></section><section className="section shell results-layout doctoralia-layout"><aside className="filters directory-filters"><h2>Refinar busca</h2><form action="/buscar"><label>Especialidade, nome ou serviço<input name="q" defaultValue={q} placeholder="Ex.: cardiologia" /></label><label>Cidade<select name="cidade" defaultValue={city}><option value="">Todas as cidades</option>{cities.map(item => <option key={item}>{item}</option>)}</select></label><label>Categoria profissional<select name="profissao" defaultValue={profession}><option value="">Todas as categorias</option>{professions.map(item => <option key={item}>{item}</option>)}</select></label><button type="submit">Aplicar filtros</button></form><div className="filter-pills"><span><ShieldCheck size={13} /> Sem agendamento</span><span><BadgeCheck size={13} /> Perfis revisados</span><span><MapPin size={13} /> Guia regional</span></div></aside><div className="results directory-results"><AdSlot code={city ? cityAdCode(city) : "DIRECTORY_TOP"} compact /><div className="result-toolbar"><div><strong>{results.length} resultados no guia</strong><span>{activeFilters.length ? activeFilters.join(" · ") : "Toda a região"}</span></div><Link href="/inclusao">Solicitar inclusão</Link></div>{results.length ? <div className="doctor-card-list">{results.map(item => <article className="doctor-card" key={item.slug}><div className="doctor-avatar"><span>{item.name.split(" ").slice(0, 2).map(part => part[0]).join("")}</span></div><div className="doctor-main"><div className="doctor-card-head"><div><p>{item.profession}</p><h2>{item.name}</h2></div><span className={item.verified ? "status-pill verified" : "status-pill pending"}>{item.verified ? <BadgeCheck size={14} /> : <ShieldCheck size={14} />}{item.verified ? "Perfil verificado" : "Em validação"}</span></div><div className="doctor-pills"><span><MapPin size={13} /> {item.city}</span><span>{item.specialty}</span><span>{item.registration}</span></div><p className="doctor-summary">{item.summary}</p><div className="service-pills">{item.services.slice(0, 4).map(service => <span key={service}>{service}</span>)}</div></div><aside className="doctor-side"><div><span className="doctor-side-label"><Building2 size={14} /> Local de atendimento</span><strong>{item.organization}</strong></div><small><Phone size={14} /> {item.phone}</small><Link href={`/profissionais/${item.slug}`}>Ver perfil <ArrowUpRight size={14} /></Link><p>Contato direto no perfil. Sem agendamento pelo portal.</p></aside></article>)}</div> : <div className="empty-state"><h2>Nenhum resultado encontrado</h2><p>Isso ajuda a priorizar novas categorias e cidades na formação da base regional.</p><Link href="/buscar">Limpar filtros</Link></div>}</div></section></main><SiteFooter /></>;
}

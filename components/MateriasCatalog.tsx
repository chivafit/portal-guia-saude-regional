...
function CompactFeature({article}:{article:EditorialArticle}){
 return <Link href={`/materias/${article.slug}`} className="content-feature" data-content-feature="compact"><span className="content-feature-art" style={article.image?{backgroundImage:`url(${article.image})`}:undefined} role="img" aria-label={article.title}/><span className="content-feature-copy"><span className="cf-cat">{article.category}</span><strong className="cf-title">{article.title}</strong><span className="cf-excerpt">{article.excerpt}</span><span className="cf-meta">{byline(article)}</span><span className="cf-action">Ler matéria <ArrowRight size={14}/></span></span></Link>;
}

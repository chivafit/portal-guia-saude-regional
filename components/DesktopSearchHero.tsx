  ...
  return (
    <section className="gs-hero">
      <div>
        ...
        <p className="gs-hero-lede">Profissionais, clínicas e serviços de saúde da região — com informação verificada, conteúdo e contato direto.</p>
      ...
      <div className="gs-hero-visual" aria-hidden={!photoUrl}>
        ...
        <div className="gs-hero-photo">{photoUrl ? <img src={photoUrl} alt="Atendimento de saúde em Piumhi" /> : null}</div>
        <div className="gs-hero-trending"><span>MAIS BUSCADO ESTA SEMANA</span>{trending.map((item) => <Link href={item.href} key={item.label}><i>{item.icon === "eye" ? <Eye size={22} /> : <HeartPulse size={22} />}</i><span><strong>{item.label}</strong><small>{item.count}</small></span></Link>)}</div>
      </div>
    </section>
  );
}

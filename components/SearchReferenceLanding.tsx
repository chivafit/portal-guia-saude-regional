...
export function SearchReferenceLanding() {
  ...
  useEffect(() => {
    document.body.classList.toggle("search-reference-active", active);
    return () => document.body.classList.remove("search-reference-active");
  }, [active]);
    ...
    <main className="search-reference-landing" aria-label="Buscar no Guia Saúde">
      <div className="search-reference-shell">
        ...
        <form className="search-reference-form" action="/buscar" role="search">
          ...
          <input name="q" autoComplete="off" enterKeyHint="search" aria-label="O que você está buscando?" placeholder="O que você está buscando?" />
        ...
        <section className="search-reference-specialties" aria-labelledby="search-specialties-title">
          <div className="search-reference-section-head"><h2 id="search-specialties-title">Especialidades</h2><Link href="/buscar?tipo=professionals&cidade=piumhi">Ver todos <span aria-hidden="true">›</span></Link></div>
          <div className="search-reference-grid">
            {specialties.map(({ label, icon: Icon, tone }) => (
              <Link key={label} className={`search-reference-card tone-${tone}`} href={`/buscar?q=${encodeURIComponent(label)}&cidade=piumhi&tipo=professionals`} aria-label={`Buscar ${label}`}>
                <span className="search-reference-card-icon"><Icon size={26} strokeWidth={1.65} /></span><strong>{label}</strong>
              </Link>
            ))}
          </div>
        </section>

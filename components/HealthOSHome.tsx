...
const shortcuts = [
  ...
  { label: "Clínicas", href: "/buscar?cidade=piumhi&tipo=services&categoria=clininas", icon: Building2 },
...
export async function HealthOSHome() {
  ...
  const featured = source.filter((item) => item.city === "Piumhi" && isFeaturedProfessional(item.slug)).slice(0, 3).map((item) => ({
    slug: item.slug, name: item.name, specialty: item.specialty, organization: item.organization, registration: item.registration, imageUrl: item.imageUrl,
  }));

  return <main className="health-os-home">
    <div className="health-os-aura health-os-aura-one" aria-hidden="true" /><div className="health-os-aura health-os-aura-two" aria-hidden="true" />
